...
2. Setup RStudio to work with Git. In RStudio, under `Tools-Global Options`, click on `Git/SVN` at the bottom. You'll need to tell RStudio where to find Git (i.e., where it is installed on your system). This folder will differ from system to system, so you'll need to look closely, but on a Mac it will probably be at `\usr\bin\git`. On Windows try: `C:/Program Files (x86)/Git/bin/git.exe`.
